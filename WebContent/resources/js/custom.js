(function($){
	...
	$(document).ready( function(){
		...
		$(".caseblock").hover(function () {
		    $(this).find("span").stop().animate({ "top": 145 }, 500)
		}, function () {
		    $(this).find("span").stop().animate({ "top": 220 + "px" }, 500)
		});

	    //init sider bars 
	    // Create a new instance of Slidebars
		var slideController = new slidebars();
	    // Initialize Slidebars
		slideController.init();
        //init left slide trigger
		$('.toggleLeftSlide').on('click', function (event) {
		    event.stopPropagation();
		    slideController.toggle('slidebar-1');
		});
		$('body').on('click', function (event) {
		    event.stopPropagation();
		    slideController.close('slidebar-1');
		});
		
		//sync content 
		$(".leftslidebar").html($(".scd_l").html());
		
		//css hack
		if($(".minilogo").width()<800&&$(".minilogo").width()>320)
		{
			//UC
			if($(".navbar-brand").width() > $(".minilogo").width()-120)
			{
				$(".navbar-brand").width($(".minilogo").width()-130);
				$("#et-slider-wrapper").height($(".minilogo").width()*0.4);
				$("#et-slider-wrapper .et-arrow-prev,#et-slider-wrapper .et-arrow-next").css('top',$(".minilogo").width()*0.2+20);
			}
		}
		if($(".minilogo").width()<321)
		{
			if($(".navbar-brand").width() > $(".minilogo").width()-80)
			{
				$(".navbar-brand").width($(".minilogo").width()-80);
			}
		}
